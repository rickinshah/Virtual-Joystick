...
module spi_master(
    ...
    output [44:0] acl_data     // 15 bit data, 5 each axis
    );
    ...
    reg [44:0] temp_DATA    = 45'b0;			// 15 bits, 5 bits for each axis		
    ...
    always @(negedge iclk)
        if(latch_data) begin		// latch 1&1/2 tick after entering state END_SPI
            temp_DATA <= { X[14:0], Y[14:0], Z[14:0] };   // latch sign bit + 4 data bits each axis
		end
